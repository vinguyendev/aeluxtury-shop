...
function createlink($module = "", $row = array()){
	...
	switch($module){
		case "category":
			$strlink = ROOT_PATH . removeTitle($row["nTitle"]) . ".html";
			break;
		case "product":
			$strlink = ROOT_PATH .  'category/'.removeTitle($row["nTitle"]) . "-id" . $row['iData'];
			break;
		case "product_cat":
			$strlink = ROOT_PATH .  'category/'.removeTitle($row["nParent"]).'/'.removeTitle($row["nTitle"]) . "-id" . $row['iData'];
			break;
		case "product_detail":
         	$strlink = ROOT_PATH  . removeTitle($row["nTitle"]) . "-p" . $row["iData"].".html";
         	break;
      	case "news":
			$strlink = ROOT_PATH . 'tin-tuc/' . removeTitle($row["nTitle"]) . ".html";
			break;
		case "news_detail":
			$strlink = ROOT_PATH .  'tin-tuc/' . removeTitle($row["nTitle"]) . "-" . $row['iData'] . ".html";
			break;
		case "static":
			$strlink = ROOT_PATH . 'static/' . removeTitle($row["nTitle"]) . '-id' . $row["iData"];
			break;
		case "recruitment":
			$strlink = ROOT_PATH . 'tuyen-dung' . $row["iData"] . '.html';
			break;
		case "album":
         	$strlink = ROOT_PATH  . 'album/' . removeTitle($row["nTitle"]) . "-id" . $row["iData"].".html";
         	break;
        case "project":
         	$strlink = ROOT_PATH  . 'project/' . removeTitle($row["nTitle"]) . "-id" . $row["iData"].".html";
         	break;
        case "sevice":
         	$strlink = ROOT_PATH  . 'dich-vu/' . removeTitle($row["nTitle"]) . "-id" . $row["iData"].".html";
         	break;
	}
    return $strlink;
}
