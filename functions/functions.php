...
function get_tag($module, $id, $type=0){

	$table			= "product_tag";
	$id_field		= "pt_product_id";
	$field_prefix	= "pt_";
	if($module == "news"){
		$table		= "news_tag";
		$id_field	= "nt_news_id";
	...
	$arrReturn		= array();
	$db_tag			= new db_query("SELECT tag_id, tag_name, tag_name_rewrite
											 FROM tags, " . $table . "
											 WHERE tag_id = " . $field_prefix . "tag_id AND " . $id_field . " = " . $id . "
											 ORDER BY " . $field_prefix . "order ASC
											 LIMIT 50");
	while($row = mysqli_fetch_assoc($db_tag->result)){
		$row["tag_link"]	= generate_tag_url($row["tag_name_rewrite"]);
		$arrReturn[$row["tag_id"]]	= $row;
	}
	$db_tag->close();
	unset($db_tag);

	// Nếu $type == 1 thì return string (trong meta keyword, edit SP)
	if($type == 1){
		$return	= "";
		foreach($arrReturn as $row) $return	.= $row["tag_name"] . ", ";
		$return	= trim($return, ", ");
	}
...
function dump_log($log_data, $log_type = "SQL_LOG"){
	//Lấy thư mục ipstore
	$log_dir 		= str_replace("functions", "log", dirname(__FILE__));
	$current_date	= date("Ymd");

	switch($log_type){
		default							: $log_file = $log_dir . "/sql/" .$current_date . "_dump_sql_log.cfn"; break;
	}
 ...
 * saving_logs()
 *
 * @param string $typeoflog : Loai log de lua chon file log tuong ung
 * @param mixed $datalog : Chuoi du lieu can log
 * @return void
 */
function saving_logs($typeoflog="", $datalog){
	$type_log = 0;

	switch ($typeoflog) {
		case "BAOKIM_ERROR_LOG"		:  $type_log = 1; break;
		case "BAOKIM_BPN_LOG"		:  $type_log = 2; break;
		case "BAOKIM_DATA_BPN_LOG" :  $type_log = 3; break;
		case "BAOKIM_PAYMENT_LOG"	:  $type_log = 4; break;
		case "MEMCACHE_LOG" 			:  $type_log = 5; break;
		case "SMS" 						:  $type_log = 6; break;
		case "FACEBOOK" 				:  $type_log = 7; break;
		case "LOANTIN" 				:  $type_log = 8; break;
		case "EMAIL" 					:  $type_log = 9; break;
		case "ORDER"					:  $type_log = 10; break;
		case "MOVE_DEAL_CATEGORY"	:  $type_log = 10; break;
	}

	if($type_log != 0){
		$query =  "INSERT INTO logs_all(log_type, log_content, log_time)
						VALUES (" . $type_log .", '" . $datalog . "'," . time() . ")";

		$insert = new db_execute($query);
		unset($insert);
	}
}
 ...
 */
function sort_desc(Integer $a, Integer $b){
	if ($a == $b) return 0;
	return ($a > $b) ? -1 : 1;
}
...
/**
 * Tạo chữ ký số dùng khi gọi API Bảo Kim
 * @param  string $method     [Phương thức POST, GET ...]
 * @param  string $url        [URL API Bảo Kim. VD: /payment/rest/account_payment_api/get_mrc_promotion_list]
 * @param  array  $getArgs    [Mảng dữ liệu truyền đi nếu dùng theo method GET.  VD: array("account_id" => "", "email" => "diepcd@gmail.com", "merchant_id" => 647)]
 * @param  array  $postArgs   [Mảng dữ liệu truyền đi nếu dùng theo method POST]
 * @param  string $priKeyFile [Nội dung của file chứ khóa private. VD: $BK_priKey = file_get_contents("/includes_dir/cucre.key")]
 * @return string             [Chữ ký được tạo ra]
 * ---------------------- Sử dụng ----------------
 * $ref_signature				= makeBaoKimAPISignature("GET","/payment/rest/account_payment_api/get_mrc_promotion_list", $array_data, array(), $BK_priKey);
 * $list_bonus->rest_url 	= "http://sandbox.baokim.vn/payment/rest/account_payment_api/get_mrc_promotion_list?signature=" . $ref_signature . "&account_id=&email=" . $account . "&merchant_id=" . $bk_merchant_id;
 * $list_bonus->array_data = $array_data;
 * $list_bonus->Set_Digest_Authentication($bk_restful_username,$bk_restful_pass);
 * $result = $list_bonus->Get_Data();
 */
function makeBaoKimAPISignature($method, $url, $getArgs=array(), $postArgs=array(), $priKeyFile){
	...
	return strval( urlencode(base64_encode($signature)) );
}
 ...
 * @return [String]       [r]
 */
function generateBreadcrumbs($iCat = 0, $htmlAdd = ''){

	$nav 	= '<div itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
               <a href="' . ROOT_PATH . '" itemprop="url">
     					<span itemprop="title">Trang chủ</span>
     				</a>
     				<i class="icon_right"></i>
            </div>';
	if($iCat > 0){
		$array_parent	= getAllParent_v2("categories_multi", "cat_id", "cat_parent_id", "cat_name_rewrite, cat_name, cat_type", $iCat);
		if(count($array_parent) > 0){
			// Sắp xếp ngược lại theo thứ tự của key -> show danh mục cao nhất trước
			$array_parent	= array_reverse($array_parent, true);
			$stt				= 1;
			foreach($array_parent as $key => $value){
				$stt++;
				$arrayLink	= array("iData" => $key, "iCat" => $key, "nTitle" => $value['cat_name_rewrite']);
				$type 		= $value['cat_type'];

				if($type == 'product') $type = "category";
				$link			= createlink($type, $arrayLink);
				$class	= "";
				if($iCat == $key) $class	= "current";
				$icon_right 	= "";
				if($stt <= count($array_parent) || $htmlAdd != "") $icon_right 	= '<i class="icon_right"></i>';
				$nav		.= '<div itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb" class="col-mm-0">
			                  <a href="' . $link . '" itemprop="url">
							        	<span itemprop="title">' . $value['cat_name'] . '</span>
							      </a>
							      ' . $icon_right . '
			               </div>';
			}
		} // End if(count($array_parent) > 0)
	}
	if($htmlAdd != "") $nav 	.= $htmlAdd;

	$nav 	= '<div class="breadcrumbs breadcrumb_url">' . $nav . '</div>';
 ...
 * @return [String]       [r]
 */
function generateBreadcrumbsBlog($iCat = 0, $htmlAdd = ''){

   $nav	= '<span itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
               <a href="/blog" itemprop="url"><span itemprop="title">Cẩm nang du lịch</span></a> ›';
   ...
   if($iCat > 0){
      ...
      if(count($array_parent) > 0){
         ...
         foreach($array_parent as $key => $value){
            ...
            $arrayLink	= array("iData" => $key, "iCat" => $key, "nTitle" => $value['cat_name_rewrite']);
            $type 		= $value['cat_type'];
            if($type == 'new') $type = "cat_blog";
            $link			= createlink($type, $arrayLink);
            $class	= "";
            if($iCat == $key) $class	= "current";
            $nav		.= '<span rel="v:child" typeof="v:Breadcrumb">
                           <a href="' . $link . '""rel="v:url">' . $value['cat_name'] . '</a> ›';
         }
