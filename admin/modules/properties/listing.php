...
$ppe_name		= getValue("ppe_name", "str", "GET", "", 1);

$sqlWhere	= "";
if($ppe_name != "" && $ppe_name != ""){
	$sqlWhere	.= " AND ppe_name LIKE '%" . $ppe_name . "%'";
}
		...
		<form action="" method="GET">
			<div class="search">
				<table class="table table_border_none">
					<tr>
						<td class="text" style="text-align: left">SEARCH: Tên thuộc tính:
							<input type="text" value="<?=$ppe_name?>" id="ppe_name" name="ppe_name" class="form-control" />
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					...
					<tr id="tr_<?=$listing["ppe_id"]?>">
						...
						<td title="Active" class="center" style="text-align: center;">
							<a onclick="update_check_ajax(<?=$listing['ppe_id']?>,'ppe_active');" id="ppe_active_<?=$listing['ppe_id']?>">
								<?if($listing['ppe_active'] == 1){
						...
						<td class="align_c"><?=date("H:i:s d/m/Y", $listing['ppe_create_time'])?></td>
						<td  class="align_c" >
							<a href="edit.php?record_id=<?=$listing['ppe_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
								<img border="0" src="../../resource/images/grid/edit.png" />
							</a>
						</td>
						<td class="align_c">
							<a onclick="if (confirm('Bạn muốn xóa bản ghi?: <?=$listing['ppe_name']?>')){ delete_branch(<?=$listing['ppe_id']?>); }" class="delete">
