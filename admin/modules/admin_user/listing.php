		...
		<table class="table table-bordered table-striped">
			...
			?>
			  <tr>
				...
				<td class="align_c"><img src="<?=$fs_imagepath?>delete.gif" border="0" onclick="if (confirm('<?=translate_text("Are you sure to delete")?>?')){ window.location.href='active.php?record_id=<?=$row["adm_id"]?>&type=adm_delete&value=<?=abs($row["adm_delete"]-1)?>&url=<?=base64_encode(getURL())?>'}" style="cursor:pointer"></td>
			  </tr>
			<? } ?>
		</table>
		<? /*---------Body------------*/ ?>
