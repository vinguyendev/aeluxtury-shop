...
if($name_branch != "" && $name_branch != ""){
	$sqlWhere	.= " AND alb_title LIKE '%" . $name_branch . "%'";
}
		...
		<form action="" method="GET">
			<div class="search">
				<table class="table table_border_none">
					<tr>
						<td class="text" style="text-align: left">SEARCH: Tên album:
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["alb_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Thông tin" valign="top">
							<?=$listing["alb_title"]?>
						</td>
						<td title="Active" class="center" style="text-align: center;">
							<a onclick="update_check_ajax(<?=$listing['alb_id']?>,'alb_active');" id="alb_active_<?=$listing['alb_id']?>">
								<?if($listing['alb_active'] == 1){
						...
						<td class="align_c"><?=$listing['alb_order']?></td>
						<td class="align_c"><?=date("H:i:s d/m/Y", $listing['alb_create_time'])?></td>
						<td  class="align_c" >
							<a href="edit.php?record_id=<?=$listing['alb_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
								<img border="0" src="../../resource/images/grid/edit.png" />
							</a>
						</td>
						<td class="align_c">
							<a onclick="if (confirm('Bạn muốn xóa bản ghi?: <?=$listing['alb_title']?>')){ delete_branch(<?=$listing['alb_id']?>); }" class="delete">
