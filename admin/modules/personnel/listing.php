...
$per_name		= getValue("per_name", "str", "GET", "", 1);

$sqlWhere	= "";
if($per_name != "" && $per_name != ""){
	$sqlWhere	.= " AND per_name LIKE '%" . $per_name . "%'";
}
				...
				<table class="table table_border_none">
					<tr>
						<td class="text" style="text-align: left">SEARCH: Tên thương hiệu:
							<input type="text" value="<?=$per_name?>" id="per_name" name="per_name" class="form-control" />
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["per_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td title="Thông tin" valign="top">
							<?=$listing["per_name"]?> &nbsp;
							<img width="100px" src="/data/personnel/<?=$listing["per_image"]?>">
						</td>
						<td valign="top">
							<?=$listing["per_pos"]?>
						</td>
						<td title="Active" class="center" style="text-align: center;">
							<a onclick="update_check_ajax(<?=$listing['per_id']?>,'per_active');" id="per_active_<?=$listing['per_id']?>">
								<?if($listing['per_active'] == 1){
						...
						<td class="align_c"><?=$listing['per_order']?></td>
						<td class="align_c"><?=date("H:i:s d/m/Y", $listing['per_create_time'])?></td>
						<td  class="align_c" >
							<a href="edit.php?record_id=<?=$listing['per_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
								<img border="0" src="../../resource/images/grid/edit.png" />
							</a>
						</td>
						<td class="align_c">
							<a onclick="if (confirm('Bạn muốn xóa bản ghi?: <?=$listing['per_name']?>')){ delete_branch(<?=$listing['per_id']?>); }" class="delete">
