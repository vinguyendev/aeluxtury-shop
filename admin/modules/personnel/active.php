...
$db_query	= new db_query("SELECT * FROM " . $fs_table . " WHERE " . $id_field . " = ". $record_id);
if($row = mysql_fetch_assoc($db_query->result)){
	//thuc hien cap nhat
	$value	= 1 - $row[$field_active];
	$db_update	= new db_execute("UPDATE " . $fs_table . " SET " . $field_active . " = ". $value ." WHERE " . $id_field . " = ". $record_id);
