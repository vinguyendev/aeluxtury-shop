...
<div id="gallery_upload_file" class="gallery_upload_file fl">
	...
	<ul id="listimg">
		...
		foreach ($arrGallery as $key => $value) {
			...
			?>
			<li id="itemGl_<?=$i?>" class='<?=$classAdd?>' title='Chọn làm ảnh đại diện'>
				<a>Ảnh <?=$i?></a>
		...
		var jqXHR		= $.ajax({
			...
			success		: function(data){
				...
				if(data.code == 1 && data.data != ""){
					...
					$("#listimg").append("<li class='" + classAdd + "' title='Chọn làm ảnh đại diện' ><input type='hidden' name='picture_data[]' value='"+data.data+"' /><img src='" + data.url + data.data  + "'></li>");
					$("#listimg li").click(function(){
						$("#listimg li").removeClass('active');
						$(this).addClass('active');
					});
