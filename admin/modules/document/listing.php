...
$doc_title		= getValue("doc_title", "str", "GET", "", 1);

$sqlWhere	= "";
if($doc_title != "" && $doc_title != ""){
	$sqlWhere	.= " AND doc_title LIKE '%" . $doc_title . "%'";
}
		...
		<form action="" method="GET">
			<div class="search">
				<table class="table table_border_none">
					<tr>
						<td class="text" style="text-align: left">SEARCH:
							<input type="text" value="<?=$doc_title?>" id="doc_title" name="doc_title" class="form-control" />
	...
	<div class="content">
		<table width="80%" cellspacing="0" cellpadding="0" class="table table-hover table-bordered">
				...
				while($listing = mysqli_fetch_assoc($db_listing->result)) {
					$No++;
					?>
					<tr id="tr_<?=$listing["doc_id"]?>">
						<td title="STT" class="bold center"><?=$No?></td>
						<td valign="top">
							<table border="0">
								<tr>
									<td>Tiêu đề: </td>
									<td><?=$listing["doc_title"]?> </td>
								</tr>
								<tr>
									<td>Link: </td>
									<td><?=$listing["doc_content"]?></td>
								</tr>
							</table>
						</td>
						<td>
							<?
							$img = getUrlImageFeedback($listing['doc_image'], "medium");
							?>
							<center><img src="<?=$img?>" style="width: 100px;"></center>
						</td>
						<td title="Active" class="center" style="text-align: center;">
							<a onclick="update_check_ajax(<?=$listing['doc_id']?>,'doc_active');" id="doc_active_<?=$listing['doc_id']?>">
								<?if($listing['doc_active'] == 1){
						...
						<td class="align_c"><?=date("H:i:s d/m/Y", $listing['doc_create_time'])?></td>
						<td  class="align_c" >
							<a href="edit.php?record_id=<?=$listing['doc_id']?>&url=<?=base64_encode($_SERVER['REQUEST_URI'])?>" title="Bạn muốn sửa đổi bản ghi" >
								<img border="0" src="../../resource/images/grid/edit.png" />
							</a>
						</td>
						<td class="align_c">
							<a onclick="if (confirm('Bạn muốn xóa bản ghi?: <?=$listing['doc_title']?>')){ delete_branch(<?=$listing['doc_id']?>); }" class="delete">
