...
$db_query = new db_query("SELECT * FROM album WHERE alb_active = 1 AND alb_id = " . $iData);
if ($row = mysqli_fetch_assoc($db_query->result)) {
	$arrPicture				= json_decode(base64_url_decode($row["alb_picture_json"]), true);
	$con_site_title		= $row["alb_title"];
	$albDescription		= $row["alb_description"];
}
else{
	exit("Album không tồn tại");
}
unset($db_query);
?>
<!DOCTYPE html>
	...
	<div class="background-overlay">
		<div id="tie-container" class="site tie-container">
			<div id="tie-wrapper">
				...
				<? include("../includes/inc_detail_album.php");?>
