...
class generate_form{
	...
	*/
	...
	Generate Insert SQL
	*/
	function generate_insert_SQL($multi=0){
	...
	*/
	function generate_insert_multi($table,$sql){
		...
		$str_field .= ')';
		$querystr	= "INSERT INTO " . $table . $str_field . " VALUES " . $sql;
		return $querystr;
	}
	/*
	Generate Update SQL
	...
	*/
	function checkjavascript(){
		$check_javascript	= '<script type="text/javascript">';
		$check_javascript	.= 'function trim(sString){while(sString.substring(0,1) == " "){sString = sString.substring(1, sString.length);}while(sString.substring(sString.length-1, sString.length) == " "){sString = sString.substring(0,sString.length-1);}return sString;}';
		$check_javascript	.= 'function checkblank(str){if(trim(str) == "") return true;else return false;}';
		$check_javascript	.= 'function isemail(email){var re = /^(\w|[^_]\.[^_]|[\-])+(([^_])(\@){1}([^_]))(([a-z]|[\d]|[_]|[\-])+|([^_]\.[^_])*)+\.[a-z]{2,3}$/i;return re.test(email);}';
		$check_javascript	.= 'function validateForm(){' . $this->strJavascript . 'document.' . $this->form_name . '.submit();}';
		$check_javascript	.= '</script>';

		echo $check_javascript;
	}
	...
	*/
	function copyRecord($tableName,$field_id,$field_name,$record_id,$key='',$listFieldFile = '',$filePath = ''){
			...
			if($listFieldFile!=''){
				...
				if($row = mysqli_fetch_assoc($db_file->result)){
